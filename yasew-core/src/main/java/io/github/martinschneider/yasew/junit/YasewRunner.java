...
public class YasewRunner extends ParentRunner<FeatureRunner> {
	...
	public YasewRunner(Class<?> clazz) throws InitializationError, IOException {
		...
		init();
		String cucumberOptions = "--tags @" + getProperty(PLATFORM_KEY)
	...
	private void loadProperties() {
		...
		String propertiesLocation = System.getProperty("yasew.properties");
		try {
	...
	private void init() {
		...
		if (platforms.length > 1) {
			throw new UnsupportedOperationException("Please specify exactly one spring profile (ANDROID, IOS or WEB).");
		}
