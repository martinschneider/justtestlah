 ...
 * Template matcher
 * 
 * This class provides methods to check whether a given image (template) is part
 * of another one (target). We use a simple (yet effective) way to detect the
 * template image in various sizes by scaling the target up and down to a
 * minimum and maximum size.
 * 
 * We return on the first match that exceeds the specific threshold (matching
 * quality). This means that it is not necessarily the best possible match.
 * 
...
public class TemplateMatcher {
	...
	public boolean match(String screenshotFile, String templateFile, double threshold) {
		...
		/**
		 * This could be improved by combining the two while loops (checking the
		 * original size first, then slightly smaller and larger images etc. instead of
		 * first scaling all the way down and then scaling up (or by using a framework
		 * that performs size-invariant template matching).
		 */
		while (image.width() > MIN_IMAGE_WIDTH) {
			...
			if (bestMatch == null || match.maxVal > bestMatch.maxVal) {
				bestMatch = match;
			}
		...
		// else
		LOG.info("Image {} does not contain image {}. The closest match has quality {}", screenshotFile, templateFile,
				bestMatch.maxVal);
		return false;
	}
